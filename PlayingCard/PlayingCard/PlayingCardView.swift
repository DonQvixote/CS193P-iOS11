...
class PlayingCardView: UIView {
    ...
    var faceCardScale: CGFloat = SizeRation.faceCardImageSizeToBoundsSize {
        didSet {
            setNeedsDisplay()
        }
    }
    ...
    override func draw(_ rect: CGRect) {
        ...
        if isFaceUp {
            if let faceCardImage = UIImage(named: randString+suit, in: Bundle(for: self.classForCoder), compatibleWith: traitCollection) {
...
//        path.fill()
    ...
    private var cornerString: NSAttributedString {
        return centeredAttributedString(randString+"\n"+suit, fontSize: cornerFontSize)
    }
...
extension PlayingCardView {
    private struct SizeRation {
        ...
        static let cornetRadiusToBoundsHeight: CGFloat = 0.06
        static let cornerOffsetToCornerRadius: CGFloat = 0.33
        static let faceCardImageSizeToBoundsSize: CGFloat = 0.75
    }
    private var cornerRadius: CGFloat {
        return bounds.size.height * SizeRation.cornetRadiusToBoundsHeight
    }
    private var cornerOffset: CGFloat {
        return cornerRadius * SizeRation.cornerOffsetToCornerRadius
    }
    private var cornerFontSize: CGFloat {
        return bounds.size.height * SizeRation.cornerFontSizeToBoundsHeight
    }
    private var randString: String {
        switch rank {
        case 1:
            return "A"
        case 2...10:
